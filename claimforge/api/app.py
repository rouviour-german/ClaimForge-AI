...
# Security (Enterprise-Level API Key setup)
API_KEY_NAME = "X-Enterprise-API-Key"
api_key_header = APIKeyHeader(name=API_KEY_NAME, auto_error=False)

async def verify_api_key(api_key_header: str = Security(api_key_header)):
    # In production, this would hit Redis/Vault
    if not api_key_header or api_key_header != "cf_enterprise_sk_live":
...
def create_app() -> FastAPI:
    ...
    # Create app
    app = FastAPI(
        title="ClaimForge AI - Enterprise Backend",
        description="AI-powered claims processing system using LangGraph + CrewAI + Pinecone + Redis",
        version="2.0.0",
        docs_url="/docs",
        redoc_url="/redoc"
    )
...
def register_routes(app: FastAPI):
    ...
    # ---------------------------------------------------------
    # HEALTH CHECK (Unsecured, High Rate Limit)
    # ---------------------------------------------------------
    ...
    # POLICY MANAGEMENT
    # ---------------------------------------------------------
    @app.post("/policies", response_model=APIResponse, tags=["Policies"], dependencies=[Depends(verify_api_key)])
    ...
    @app.get("/policies/{policy_number}", response_model=PolicyResponse, tags=["Policies"], dependencies=[Depends(verify_api_key)])
    @limiter.limit("60/minute")
    ...
    # CLAIM SUBMISSION
    # ---------------------------------------------------------
    @app.post("/claims", response_model=APIResponse, tags=["Claims"], dependencies=[Depends(verify_api_key)])
